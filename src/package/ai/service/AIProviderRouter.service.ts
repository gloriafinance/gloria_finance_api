      ...
      try {
        ...
        if (
          ...
          try {
            ...
            repairRequestSent = true

            await this.consumeRequestsBudget(candidate.provider, 1)
            const repairedTokens = this.estimateTokens(
              JSON.stringify(repairedExecution.data)
            )
            await this.consumeTokensBudget(candidate.provider, repairedTokens)

            if (repairedExecution.meta) {
              await this.applyExternalQuota(
                candidate.provider,
                repairedExecution.meta
              )
            }
            ...
            this.logger.info(
              `AI Router success provider=${candidate.provider} via=repair_pass attempts=${attempts} durationMs=${Date.now() - start}`
            )

            return repairedValidated
          } catch (repairError) {
            if (!repairRequestSent) {
              await this.consumeRequestsBudget(candidate.provider, 1)
            }
            ...
            this.logger.error(
              `AI Router repair pass failed provider=${candidate.provider} code=${mapped.code} status=${mapped.status} message=${mapped.rawMessage}`
            )
          }
        }
