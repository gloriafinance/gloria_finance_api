...
memberRoute.post("/", PermissionMiddleware, Can("members", "manage"), async (req, res) => {
  await MemberController.createOrUpdate(req.body as MemberRequest, res)
})

memberRoute.get("/list", PermissionMiddleware, Can("members", "manage"), async (req, res) => {
  const params = req.query as unknown as MemberPaginateRequest
  await MemberController.list(params, res)
})

memberRoute.get("/all", PermissionMiddleware, Can("members", "manage"), async (req, res) => {
